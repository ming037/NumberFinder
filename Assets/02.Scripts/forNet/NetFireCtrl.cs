...
public class NetFireCtrl : NetworkBehaviour 
{
	...
	{
		ItemSpawner.GetComponent<NetItemSpawner> ().itemcnt -= 1;
		...
		//print (destroyindex);
		this.gameObject.GetComponent<NetPlayerSetup> ().AddtoList(destroyindex);
	}
            ...
            if (Time.time >= nextFire) //Time.time은 시간 누적
            {
				...
				for (int i = 0; i < countraycast; i++)
				{
					...
					else if (Physics.Raycast (firePos.position + i * addpos, firePos.forward, out hit, raycastdis, 1 << 9)) //메인 보드 일 때
					{
						...
						UIControl.GetComponent<NetUICtrl>().SetBoardCanvasOn();
					}
				}
